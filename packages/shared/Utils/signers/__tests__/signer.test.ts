...
describe("hSMSigner Test Suit", () => {
  ...
  it(
    "should initialise a kernel account with custom signer and sign a transaction",
    async () => {
  ...
  it(
    "should generate smart accounts based on index, and get owner",
    async () => {
      const entryPoint = getEntryPoint("0.7");
      const kernelVersion = KERNEL_V3_1;
  ...
  it(
    "should create a EIP7702 account from eoa",
    async () => {
      const entryPoint = getEntryPoint("0.7");
      ...
      console.log(
        "Kernel version contract and address",
        kernelVersion,
        KERNEL_V3_3,
        kernelAddress,
      );
